...
EspalexaDevice *Espalexa::getDevice(int pos)
{
  if (pos >= currentDeviceCount)
    return NULL;
  return devices[pos];
}

void Espalexa::removeAllDevices()
{
...
int Espalexa::Espalexa::getDeviceCount()
{
...
{
  if (deviceId < 1 || deviceId > currentDeviceCount)
    return "{}"; //error
...
bool Espalexa::handleAlexaApiCall(String req, String body) //basic implementation of Philips hue api functions needed for basic Alexa control
{
  ...
  if (req.indexOf("state") > 0) //client wants to control light
  {
    ...
    DEBUG("ls");
    DEBUGLN(tempDeviceId);
    if (body.indexOf("bri") > 0)
    {
      alexaDim(tempDeviceId, body.substring(body.indexOf("bri") + 5).toInt());
      return true;
    }
...
void Espalexa::alexaOn(uint8_t deviceId)
{
  ...
  devices[deviceId - 1]->doCallback(deviceId - 1);
}
...
void Espalexa::respondToSearch()
{
  ...
  String response =
      ...
      "LOCATION: http://" +
      String(s) + ":80/description.xml\r\n"
                  "SERVER: FreeRTOS/6.0.5, UPnP/1.0, IpBridge/1.17.0\r\n" // _modelName, _modelNumber
                  "hue-bridgeid: " +
      escapedMac + "\r\n"
                   "ST: urn:schemas-upnp-org:device:basic:1\r\n" // _deviceType
                   "USN: uuid:2f402f80-da50-11e1-9b23-" +
      escapedMac + "::upnp:rootdevice\r\n" // _uuid::_deviceType
                   "\r\n";
...
void EspalexaDevice::setName(String deviceName)
{
  _deviceName = deviceName;
}
...
void EspalexaDevice::doCallback(int index)
{
  _callback(_val, _val_last, _deviceName, index);
}
